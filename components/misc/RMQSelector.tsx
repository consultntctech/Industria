...
const RMQSelector = ({material, quantity, weight, inputId, onChangeInput, name, className, ...props}:RMQSelectorProps) => {
  ...
  return (
    <div className={`border-[0.5px] flex flex-row items-end gap-2 border-gray-200 p-2 rounded relative ${className}`} {...props}>
        ...
        <input className="border-b border-gray-300 outline-none text-center w-24" required value={qtyValue} placeholder="Quantity" name={`qty-${inputId}`} id={`qty-${inputId}`} onChange={onChangeInput} type="number" max={material?.qAccepted} min={0} />
